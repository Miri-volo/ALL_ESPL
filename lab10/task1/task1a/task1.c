...
void readlf_lPrinter(Elf32_Phdr* header, int index) {
    ...
    if (header->p_flags == PF_X) {printf("E   ");} 
    else if (header->p_flags == PF_W) {printf("W   ");} 
    else if (header->p_flags == PF_W + PF_X) {printf("WE  ");}
    else if ( header->p_flags == PF_R) {printf("R   ");} 
    else if (header->p_flags == PF_R + PF_X) {printf("RE  ");} 
    else if (header->p_flags ==PF_R + PF_W) {printf("RW  ");}
    else if (header->p_flags ==PF_R + PF_W + PF_X) {printf("RWE ");}
    else{printf("    ");}
