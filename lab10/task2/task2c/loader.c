...
int foreach_phdr(void *map_start, void (*func)(Elf32_Phdr *,int), int arg){
    ...
    for (i=0; i<header->e_phnum; i++) {
        func((Elf32_Phdr*)((void*)pointer + i*header->e_phentsize), arg);
    }
    return 0;
}
...
void readlf_lPrinter(Elf32_Phdr* header, int _) {
...
int protFlags(int pFlags) {
    int flags = pFlags & PF_R ? PROT_READ : 0;
    flags |= pFlags & PF_W ? PROT_WRITE : 0;
    return flags | (pFlags & PF_X ? PROT_EXEC : 0);
}
...
void load_phdr(Elf32_Phdr* phdr, int fd) {
    ...
    if (
        ...
        readlf_lPrinter(phdr, 0);
    } else {
        fprintf(stderr, "Error when loading program header.\n");
    }
}
...
int main(int argc, char **argv)
{
    ...
    foreach_phdr(s->mapStart, &load_phdr, s->currentFD);
    Elf32_Ehdr* ehdr = s->mapStart;
    return startup(argc - 1, argv + 1, ehdr->e_entry);
}
