...
void relocationTables(State* s) {
    if (s->currentFD <= 0) {
        fprintf(stderr, "Error: Must examine ELF file before printing relocation table.\n");
        return;
    }
    int i;
    int k;
    ...
    Elf32_Shdr * sectionHeaders = s->mapStart + header->e_shoff;
    Elf32_Shdr* stringTableHeader = &sectionHeaders[header->e_shstrndx];
    char* stringTable = s->mapStart + stringTableHeader->sh_offset;
    for(i=0; i<header->e_shnum;i++){                
        if(sectionHeaders[i].sh_type == SHT_DYNSYM){
            dynsym = (Elf32_Sym *)((char *)s->mapStart + sectionHeaders[i].sh_offset);
            dynstr = (char *) (s->mapStart + sectionHeaders[sectionHeaders[i].sh_link].sh_offset);
            break;
        }
    }
    for(i = 0 ; i < header->e_shnum ; i++){
        if(sectionHeaders[i].sh_type == SHT_REL){
            relocationTable = (Elf32_Rel*) (s->mapStart + sectionHeaders[i].sh_offset);
            printf("\nRelocation section '%s' at offset %x contains %d entries:\n",
                stringTable + sectionHeaders[i].sh_name,
                sectionHeaders[i].sh_offset,
                sectionHeaders[i].sh_size / sizeof(Elf32_Rel));
            printf("Offset   Info     Type Sym.Value Sym.Name\n");
            for(k=0; k<sectionHeaders[i].sh_size / sizeof(Elf32_Rel); k++){
                symbolIndex = ELF32_R_SYM(relocationTable[k].r_info);
                printf("%08x %08X %-4d %08X  %s\n",
                    relocationTable[k].r_offset, 
                    relocationTable[k].r_info, 
                    ELF32_R_TYPE(relocationTable[k].r_info), 
                    dynsym[symbolIndex].st_value,
                    dynstr + dynsym[symbolIndex].st_name);
            }
        }
    }
}
