...
void kill_virus(char *fileName, int signitureOffset, int signitureSize){
    ...
    fseek(file,4+signitureOffset,SEEK_SET);
...
link * fix_file(link* list){
    char fname[50];
    char fnameHelper[50];
    char startingByte[50];
    char startingByteHelper[50];
    char size[50];
    char sizeHelper[50];
    printf("Please enter file name: \n");
    fgets(fnameHelper, sizeof(fnameHelper), stdin);
    ...
    fgets(startingByteHelper, sizeof(startingByteHelper), stdin);
    sscanf(startingByteHelper, "%s", startingByte);
    printf("Please enter the size of the virus signature: \n");
    fgets(sizeHelper, sizeof(sizeHelper), stdin);
    sscanf(sizeHelper, "%s", size);
    kill_virus(fname, atoi(startingByte), atoi(size));
    return list;
}
...
int main(int argc, char **argv){
  ...
  char tempInput[10];
  link *list = NULL;
  struct fun_desc menu[] = { { "Load signatures", load_signatures}, { "Print signatures", print_signatures}, { "Detect viruses", call_detect_virus }, { "Fix file", fix_file}, { "Quit", quit} ,{ NULL, NULL } };
