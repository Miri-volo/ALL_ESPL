...
void memoryDisplay(state* s){
    ...
    int addr;
    int rows;
    printf("Enter count and address to start at:\n");
    fgets(input,sizeof(input),stdin);
    sscanf(input, "%d%X", &rows, &addr);
    if (s->display_mode == 0) {
        printf("Decimal\n=======\n");
...
void saveIntoFile(state* s){
    ...
    if (0 != fseek(file, target * s->unit_size, SEEK_SET)) {
    ...
    unsigned char* buf_begin = s->mem_buf + (s->mem_count * source);
